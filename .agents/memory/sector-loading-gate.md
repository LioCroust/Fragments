...
**How to apply:** Keep the loading overlay above the cockpit, HUD, arena, and controls. Preload shared sprites once, load the current or next background on demand, release the previous background after a transition, then hide the overlay and enqueue the sector-start banner. On native startup, retain a bounded fallback that initializes the remembered/tutorial sector and exposes the touch-to-start state if the normal async bootstrap has not progressed.