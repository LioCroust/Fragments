...
**How to apply:** Keep the loading overlay above the cockpit, HUD, arena, and controls. Preload shared sprites once, load the current or next background on demand, release the previous background after a transition, then keep the full-screen launch artwork visible until the user taps to start and only then enqueue the sector-start banner. Native startup now uses the same user-gated artwork flow as web once Skia and the required assets are ready.