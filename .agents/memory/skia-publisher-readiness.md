...
The native Skia publisher must be installed as soon as the dynamic arena mounts, even when some sprite images are still unavailable. The picture builder must tolerate missing images and draw the gameplay-critical geometry procedurally. Commit the first picture through the component prop as well as JSI, and give the native view explicit position/size instead of combining absoluteFill with width/height.
